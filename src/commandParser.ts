...
const BONE_ALIASES: Record<string, string> = {
  head: 'head', neck: 'neck', chest: 'chest', 'upper chest': 'upperChest', hips: 'hips', spine: 'spine',
  'left shoulder': 'leftShoulder', 'right shoulder': 'rightShoulder',
  'left upper arm': 'leftUpperArm', 'right upper arm': 'rightUpperArm', 'left arm': 'leftUpperArm', 'right arm': 'rightUpperArm',
  'left lower arm': 'leftLowerArm', 'right lower arm': 'rightLowerArm', 'left forearm': 'leftLowerArm', 'right forearm': 'rightLowerArm',
  'left hand': 'leftHand', 'right hand': 'rightHand',
  'left upper leg': 'leftUpperLeg', 'right upper leg': 'rightUpperLeg', 'left thigh': 'leftUpperLeg', 'right thigh': 'rightUpperLeg',
  'left lower leg': 'leftLowerLeg', 'right lower leg': 'rightLowerLeg', 'left shin': 'leftLowerLeg', 'right shin': 'rightLowerLeg',
  'left foot': 'leftFoot', 'right foot': 'rightFoot',
};

function findBoneAlias(text: string): string | undefined {
  return Object.keys(BONE_ALIASES).sort((a, b) => b.length - a.length).find((key) => text.includes(key));
}
...
function bodyDelta(text: string, fallback = 10): number {
  return clamp(Math.abs(numberFrom(text, fallback)) / 100, 0.03, 0.3);
}
...
export function parseDirectorCommand(input: string): ParsedDirectorCommand {
  ...
  const bodyPresets: Array<[RegExp, string, string]> = [
    [/\b(female body|woman body|feminine body|body female|body feminine)\b/, 'feminine', 'Realistic adult female body preset.'],
    [/\b(male body|man body|masculine body|body male|body masculine)\b/, 'masculine', 'Realistic adult male body preset.'],
    [/\b(curvy body|body curvy|hourglass body)\b/, 'curvy', 'Curvy adult body preset.'],
    [/\b(athletic body|body athletic|fit body|sporty body)\b/, 'athletic', 'Athletic adult body preset.'],
    [/\b(slim body|body slim|slender body|lean body)\b/, 'slim', 'Slim adult body preset.'],
    [/\b(natural body|adult body|normal body|reset body)\b/, 'natural', 'Natural adult body preset.'],
  ];
  for (const [pattern, preset, summary] of bodyPresets) {
    if (pattern.test(text)) {
      actions.push({ type: 'expression', name: `body:${preset}`, value: 1 });
      return { raw: input, actions, summary };
    }
  }

  const adjustments: Array<[RegExp, string, number, string]> = [
    [/\b(wider|broader|bigger) shoulders?\b/, 'shoulders', 1, 'Broaden shoulders.'],
    [/\b(narrower|smaller) shoulders?\b/, 'shoulders', -1, 'Narrow shoulders.'],
    [/\b(wider|broader|bigger) hips?\b/, 'hips', 1, 'Widen hips.'],
    [/\b(narrower|smaller) hips?\b/, 'hips', -1, 'Narrow hips.'],
    [/\b(wider|thicker|bigger) waist\b/, 'waist', 1, 'Widen waist.'],
    [/\b(smaller|narrower|slimmer) waist\b/, 'waist', -1, 'Narrow waist.'],
    [/\b(bigger|broader|wider) chest\b/, 'chest', 1, 'Increase chest build.'],
    [/\b(smaller|narrower) chest\b/, 'chest', -1, 'Reduce chest build.'],
    [/\b(more muscular|bigger build|heavier build)\b/, 'build', 1, 'Increase overall build.'],
    [/\b(less muscular|smaller build|lighter build)\b/, 'build', -1, 'Reduce overall build.'],
    [/\blonger arms?\b/, 'arms', 1, 'Lengthen arms.'], [/\bshorter arms?\b/, 'arms', -1, 'Shorten arms.'],
    [/\blonger legs?\b/, 'legs', 1, 'Lengthen legs.'], [/\bshorter legs?\b/, 'legs', -1, 'Shorten legs.'],
  ];
  for (const [pattern, part, direction, summary] of adjustments) {
    if (pattern.test(text)) {
      actions.push({ type: 'expression', name: `bodyAdjust:${part}:${bodyDelta(text) * direction}`, value: 1 });
      return { raw: input, actions, summary };
    }
  }
  ...
  const expressionMap: Array<[RegExp, string]> = [
    [/\b(smile|happy)\b/, 'happy'], [/\b(angry|mad)\b/, 'angry'], [/\b(sad|unhappy)\b/, 'sad'],
    [/\b(surprised|surprise|shocked)\b/, 'surprised'], [/\b(relaxed|calm)\b/, 'relaxed'], [/\bneutral\b/, 'neutral'], [/\bblink\b/, 'blink'],
  ];
  ...
  if ((text.includes('turn head') || text.includes('look')) && (text.includes('left') || text.includes('right'))) {
    ...
    actions.push({ type: 'bone', bone: 'head', axis: 'y', degrees: text.includes('right') ? -amount : amount });
  ...
  if ((text.includes('look') || text.includes('head')) && (text.includes('up') || text.includes('down'))) {
    ...
    actions.push({ type: 'bone', bone: 'head', axis: 'x', degrees: text.includes('up') ? -amount : amount });
  ...
  if ((text.includes('raise') || text.includes('lift')) && text.includes('arm')) {
    ...
    actions.push({ type: 'bone', bone: side === 'left' ? 'leftUpperArm' : 'rightUpperArm', axis: 'z', degrees: side === 'left' ? -amount : amount });
  ...
  return { raw: input, actions, summary: 'Command not recognised yet. Use a body preset, precise bone/axis command, or one of the built-in natural commands.' };
}
